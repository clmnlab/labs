function write_Regressor_IF(subj_idn,day_n)
    total_n=6;
    switch day_n
        case 1
            earlylate='E';
    ...
    if subj_idn < 10 
        if exist(strcat('regressor/G7T_IF',earlylate,'0',num2str(subj_idn),'/onset'),'dir')==0
            mkdir(strcat('regressor/G7T_IF',earlylate,'0',num2str(subj_idn),'/onset'))
        end
        result_dir=strcat('regressor/G7T_IF',earlylate,'0',num2str(subj_idn),'/onset');
        LSSresult_dir=strcat('regressor/G7T_IF',earlylate,'0',num2str(subj_idn));
    else
        if exist(strcat('regressor/G7T_IF',earlylate,num2str(subj_idn),'/onset'),'dir')==0
            mkdir(strcat('regressor/G7T_IF',earlylate,num2str(subj_idn),'/onset'))
        end
        result_dir=strcat('regressor/G7T_IF',earlylate,num2str(subj_idn),'/onset');
        LSSresult_dir=strcat('regressor/G7T_IF',earlylate,num2str(subj_idn));
    end
    
    regressor=extract_Regressor_IF(subj_idn,day_n,1);
    ...
    for session_n = 2:total_n
        regressor=extract_Regressor_IF(subj_idn,day_n,session_n);
    ...
    for run = 1 : total_n
        regressor=extract_Regressor_IF(subj_idn,day_n,run);
        ...
        LSS_File_run = fopen(strcat(LSSresult_dir,'/LSStime_run0',num2str(run),'.txt'),'w');
