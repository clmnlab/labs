function write_Regressor(subj_idn,total_n)
    if subj_idn < 10 
        if exist(strcat('regressor/G7T_TR0',num2str(subj_idn),'/onset'),'dir')==0
            mkdir(strcat('regressor/G7T_TR0',num2str(subj_idn),'/onset'))
        end
        result_dir=strcat('regressor/G7T_TR0',num2str(subj_idn),'/onset');
        result_LSSdir=strcat('regressor/G7T_TR0',num2str(subj_idn));
    else
        if exist(strcat('regressor/G7T_TR',num2str(subj_idn),'/onset'),'dir')==0
            mkdir(strcat('regressor/G7T_TR',num2str(subj_idn),'/onset'))
        end
        result_dir=strcat('regressor/G7T_TR',num2str(subj_idn),'/onset');
        result_LSSdir=strcat('regressor/G7T_TR',num2str(subj_idn));
    end
    regressor=extract_Regressor(subj_idn,1,total_n);
    ...
    for session_n = 2:total_n
        regressor=extract_Regressor(subj_idn,session_n,total_n);
    ...
    for run = 1 : total_n
        regressor=extract_Regressor(subj_idn,run,total_n);
        ...
        LSS_File_run = fopen(strcat(result_LSSdir,'/LSStime_run0',num2str(run),'.txt'),'w');
