...
from clmnlab_libs.mvpa_toolkits import run_spatiotemporal_searchlight, get_full_mask, load_5d_fmri_image
...
if __name__ == '__main__':
    ...
    for subj in subj_list:
        ...
        for run in runs:
            ...
            data = img.get_data().reshape((72, 85, 72, 97, 10))
            new_img = nilearn.image.new_img_like(img, data[:, :, :, 1:, :])

            X = new_img
            y, chance_level = get_label_list_and_chance_level()
            cv = GroupKFold(8)

            searchlight_img = run_spatiotemporal_searchlight(
                mask_img, X, y, group, cv=cv, radius=radius, estimator=estimator, chance_level=chance_level)

            searchlight_img.to_filename(
                result_dir + 'spatiotemporal-%s/%s_r%d_within-run%d_%s_%s.nii.gz' % (
                    label, subj, radius, run, estimator_name, label)
            )
