function perf = calcRegressors(subjID,option)
cd('/Users/sskim/Documents/MATLAB/GA/data');
load(sprintf([subjID '-fmri']));
dst_dir = fullfile('/Users/sskim/Documents/Research/AFNI/GA/data',subjID,'regressors');
if exist(dst_dir,'dir')==0 mkdir(dst_dir);end
nTrials = [8 97*ones(1,7)];
nS = 300; %% Number of sample of 1 trial for run 2~7
idx1 = find(fb==1);
...
for r=1:R
    for n=1:N/R          %% 145
        data_chunk_filtered(n,r,:,:) = filteredData(:,nS*N/R *(r-1)+nS*(n-1)+1:nS*N/R *(r-1)+nS*n);
        data_chunk(n,r,:,:) = allData(:,nS*N/R *(r-1)+nS*(n-1)+21:nS*N/R *(r-1)+nS*n+20);
        avcost(n,r,:) = sum(abs(diff(squeeze(data_chunk_filtered(n,r,:,:))')));
        v2cost(n,r,:) = sum((diff(squeeze(data_chunk_filtered(n,r,:,:))').^2));
        tpos(n,r,:) = convert_ID(targetID(N/R *(r-1)+n))*boxSize;
        XY(n,r,:,:) = allXY(:,nS*N/R*(r-1)+nS*(n-1)+1:nS*N/R *(r-1)+nS*n);
        errR(n,r,:) = sqrt(sum((squeeze(XY(n,r,:,:))-repmat(squeeze(tpos(n,r,:)),1,nS)).^2));
        for k=1:14 
            expl(n,r,k) = std(squeeze(data_chunk(n,r,k,:)));
            explTR(2*(n-1)+1,r,k) = std(squeeze(data_chunk(n,r,k,1:nS/2)));
            explTR(2*n,r,k) = std(squeeze(data_chunk(n,r,k,nS/2+1:nS)));
            idx = find(cnt_hit_all(N/R*(r-1)+n,:)==0);
            idx1 = find(cnt_hit_all(N/R*(r-1)+n,1:nS/2)==0);
            idx2 = find(cnt_hit_all(N/R*(r-1)+n,nS/2+1:nS)==0);
            if length(idx) < 30  idx = find(cnt_hit_all(N/R*(r-1)+n,:)==1); end;
            if length(idx1) < 30  idx1 = find(cnt_hit_all(N/R*(r-1)+n,1:nS/2)==1); end;
            if length(idx2) < 30  idx2 = find(cnt_hit_all(N/R*(r-1)+n,nS/2+1:nS)==1); end;
            expl2(n,r,k) = std(squeeze(data_chunk(n,r,k,idx)));
            explTR2(2*(n-1)+1,r,k) = std(squeeze(data_chunk(n,r,k,idx1)));
            explTR2(2*n,r,k) = std(squeeze(data_chunk(n,r,k,nS/2+idx2)));
        end
    end
end
...
% 
    ...
    idx1_2 = setdiff(idx1,idx1_1);  %% FB from r05, r06
